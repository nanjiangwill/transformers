    ...
    def generate(
        ...
        **kwargs,
        ...
        is_greedy_gen_mode = (
            ...
            and generation_config.do_sample is False
            and generation_config.gumbel is False
            and not is_constraint_gen_mode
            and not is_contrastive_search_gen_mode
        )
        is_greedy_gumbel_gen_mode = (
            (generation_config.num_beams == 1)
            and (generation_config.num_beam_groups == 1)
            and generation_config.do_sample is False
            and generation_config.gumbel is True
            and not is_constraint_gen_mode
            and not is_contrastive_search_gen_mode
        )
        is_sample_gen_mode = (
        ...
        if is_greedy_gen_mode:
        ...
        elif is_greedy_gumbel_gen_mode:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    "num_return_sequences has to be 1 when doing greedy search, "
                    f"but is {generation_config.num_return_sequences}."
            ...
            # 11. run greedy search
            return self.greedy_search_gumbel(
                input_ids,
                gumbel_hard=generation_config.gumbel_hard,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                eos_token_id=generation_config.eos_token_id,
    ...
    def greedy_search_gumbel(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # gumbel_softmax
            next_tokens = nn.functional.gumbel_softmax(next_tokens_scores, tau=1, hard=True)
            if gumbel_hard==True:
                tokens_one_hot.append(next_tokens)
            else:
                tokens_one_hot.append(nn.functional.gumbel_softmax(next_tokens_scores, tau=1, hard=gumbel_hard))
            
            next_tokens = next_tokens@torch.Tensor([i for i in range(next_token_logits.shape[-1])]).to(next_tokens.device)
        ...
        if return_dict_in_generate:
            ...
            return input_ids, torch.cat(tokens_one_hot, dim=0)
